...
{
	strncpy(info->sig, AX8817XX_SIGNATURE, strlen(AX8817XX_SIGNATURE));
	return 0;
}
...
int ioctl_read_eeprom(struct usbnet *dev, AX_IOCTL_COMMAND *info)
{
	...
	if (info->type == 0) //EEPROM
	{
		for (i = 0; i < info->size; i++) {
			//retval = ax88179_read_cmd(dev, AX_ACCESS_EEPROM, i, 1, 2, &tmp);

			if (ax88179_write_cmd(dev, AX_ACCESS_MAC, AX_SROM_ADDR, 1, 1, &i) < 0) 
				return -EINVAL;
			...
			if (ax88179_read_cmd(dev, AX_ACCESS_MAC, AX_SROM_DATA_LOW, 2, 2, &tmp) < 0) 
				return -EINVAL;	
	...
	else //eFuse
	{
		for (i = 0; i < info->size; i++){
			...
			if (ax88179_read_cmd(dev, AX_ACCESS_EFUSE, i, 1, 2, &tmp) < 0) 
				return -EINVAL;	
			*(info->buf + i) = be16_to_cpu(tmp);
		}
	}
...
int ioctl_write_eeprom(struct usbnet *dev, AX_IOCTL_COMMAND *info)
{
	...
	{			
		if (*(info->buf) & 0x01) {
			printk(KERN_INFO "Cannot be set to muliticast MAC address, bit0 of Node ID-0 cannot be set to 1. \r\n");
			return -EINVAL;
		}

		// checksum
		csum = (*(info->buf + 3)&0xff) + ((*(info->buf + 3) >> 8)&0xff) +(*(info->buf + 4)&0xff) + ((*(info->buf + 4) >> 8)&0xff);
		csum = 0xff- ((csum >> 8) + (csum & 0xff));
		data = ((*(info->buf + 5)) & 0xff) | (csum << 8);
		*(info->buf + 5) = data;

		for (i = 0; i < info->size; i++){		
			data = cpu_to_be16(*(info->buf + i));
			if (ax88179_write_cmd(dev, AX_ACCESS_EEPROM, i, 1, 2, &data) < 0) 
				return -EINVAL;		
		}
	}
	...
	{			
		...
		// checksum
		for (i = 0; i < info->size; i++)
			csum += (*(info->buf + i)&0xff) + ((*(info->buf + i) >> 8)&0xff);
	
		csum -= ((*(info->buf + 0x19) >> 8) & 0xff);
		while (csum > 255)
			csum = (csum & 0x00FF) + ((csum >> 8) & 0x00FF);		
		csum = 0xFF - csum;

		data = ((*(info->buf + 0x19)) & 0xff) | (csum << 8);		
		*(info->buf + 0x19) = data;

		if (ax88179_write_cmd(dev, AX_WRITE_EFUSE_EN, 0, 0, 0, NULL) < 0) 
			return -EINVAL;		

		for (i = 0; i < info->size; i++){		
			data = cpu_to_be16(*(info->buf + i));
			if (ax88179_write_cmd(dev, AX_ACCESS_EFUSE, i, 1, 2, &data) < 0) 
				return -EINVAL;		
		}

		if (ax88179_write_cmd(dev, AX_WRITE_EFUSE_DIS, 0, 0, 0, NULL) < 0) 
			return -EINVAL;	
	}
	else if (info->type == 2)  //check efuse existed or not
	{
		//printk(KERN_INFO "CHECK efuse empty or not?");
		if (ax88179_read_cmd(dev, AX_ACCESS_EFUSE, 0, 1, 2, &data) < 0) 
				return -EINVAL;	
...
static int ax88179_ioctl (struct net_device *net, struct ifreq *rq, int cmd)
{
	...
	case AX_PRIVATE :
		...
		{
			printk(KERN_INFO "ioctl_tbl, return -EFAULT");
			return -EFAULT;
		}	
